/*
 * SysInfo - sysinfo plugin for ARABICON
...
#define _(x) arabicon_gettext(ph,x)
...
get_client (void)
{
	return g_strdup_printf ("ARABICON %s", arabicon_get_info(ph, "version"));
}
...
print_summary (gboolean announce)
{
	...
	arabicon_commandf (ph, "%s %s", announce ? "SAY" : "ECHO", output);
...
print_info (char *info, gboolean announce)
{
	...
	for (i = 0; hwinfos[i].name != NULL; i++)
	{
		if (!g_ascii_strcasecmp (info, hwinfos[i].name))
		{
			...
			if (str)
			{
				arabicon_commandf (ph, "%s \002%s\002: %s", announce ? "SAY" : "ECHO",
									hwinfos[i].title, str);
				g_free (str);
			}
			else
				arabicon_print (ph, _("Sysinfo: Failed to get info. Either not supported or error."));
			return;
		}
	}

	arabicon_print (ph, _("Sysinfo: No info by that name\n"));
}

static gboolean
sysinfo_get_bool_pref (const char *pref, gboolean def)
{
	int value = arabicon_pluginpref_get_int (ph, pref);
...
sysinfo_set_pref_real (const char *pref, char *value, gboolean def)
{
	if (value && value[0])
	{
		...
		arabicon_pluginpref_set_int (ph, pref, i != 0);
		arabicon_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref, i != 0);
	}
	else
	{
		arabicon_printf (ph, _("Sysinfo: %s is set to: %d\n"), pref,
...
sysinfo_set_pref (char *key, char *value)
{
	if (!key || !key[0])
	{
		arabicon_print (ph, _("Sysinfo: Valid settings are: announce and hide_* for each piece of information. e.g. hide_os. Without a value it will show current (or default) setting.\n"));
		return;
	}
	...
	arabicon_print (ph, _("Sysinfo: Invalid variable name\n"));
}
...
sysinfo_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	channel_type = arabicon_list_int (ph, NULL, "type");
...
int
arabicon_plugin_init (arabicon_plugin *plugin_handle, char **plugin_name, char **plugin_desc, char **plugin_version, char *arg)
{
	ph = plugin_handle;
	*plugin_name = name;
	*plugin_desc = desc;
	*plugin_version = version;
	...
	arabicon_hook_command (ph, "SYSINFO", HEXCHAT_PRI_NORM, sysinfo_cb, sysinfo_help, NULL);

	arabicon_command (ph, "MENU ADD \"Window/Send System Info\" \"SYSINFO\"");
	arabicon_printf (ph, _("%s plugin loaded\n"), name);
	return 1;
}

int
arabicon_plugin_deinit (void)
{
	arabicon_command (ph, "MENU DEL \"Window/Display System Info\"");
	arabicon_printf (ph, _("%s plugin unloaded\n"), name);
	return 1;
}
