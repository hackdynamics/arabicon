...
#   gui_entry
#     This is arabicon_plugin pointer that is used to remove the script from
...
sub unload {
	...
	if( $pkg_info ) {	
		...
		return ARABICON::EAT_ALL;
	} else {
		ARABICON::print( qq{"$file" is not loaded.\n} );
		return ARABICON::EAT_NONE;
	}
}
...
sub reload_all {
	my @dirs = ARABICON::get_info( "configdir" );
...
sub evaluate {
	...
	ARABICON::print $@ if $@; #print warnings

	local $Data::Dumper::Sortkeys = 1;
	local $Data::Dumper::Terse    = 1;

	if (@results > 1) {
		ARABICON::print Dumper \@results;
	}
	elsif (ref $results[0] || !$results[0]) {
		ARABICON::print Dumper $results[0];
	}
	else {
		ARABICON::print $results[0];
	}

	return ARABICON::EAT_HEXCHAT;
};
...
sub find_external_pkg {
	...
	while( my @frame = caller( $level ) ) {
		return @frame if $frame[0] !~ /(?:^IRC$|^ARABICON)/;
		$level++;
	}
	return;
}
...
sub find_pkg {
	...
	while( my ($package, $file, $line) = caller( $level ) ) {
		return $package if $package =~ /^ARABICON::Script::/;
		$level++;
	}
