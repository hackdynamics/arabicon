...
static arabicon_plugin *ph;		  /* plugin handle */
...
perl_auto_load (void *unused)
{
	...
	xdir = arabicon_get_info (ph, "configdir");

	/* don't pollute the filesystem with script files, this only causes misuse of the folders
	 * only use ~/.config/arabicon/addons/ and %APPDATA%\ARABICON\addons */
...
typedef struct
{
	SV *callback;
	SV *userdata;
	arabicon_hook *hook;   /* required for timers */
	arabicon_context *ctx; /* allow timers to remember their context */
...
execute_perl (SV * function, char *args)
{
	...
	SPAGAIN;
	if (SvTRUE (ERRSV)) {
		arabicon_printf(ph, "Perl error: %s\n", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		/* remove undef from the top of the stack */
	} else if (count != 1) {
		arabicon_printf (ph, "Perl error: expected 1 value from %s, "
...
static SV *
list_item_to_sv ( arabicon_list *list, const char *const *fields )
{
	HV *hash = newHV();
	SV *field_value;
	const char *field;
	...
	while (fields[field_index] != NULL) {
		...
		case 's':
			field = arabicon_list_str (ph, list, field_name);
			if (field != NULL) {
			...
			break;
		case 'p':
			field_value = newSViv (PTR2IV (arabicon_list_str (ph, list,
																	 field_name)));
			break;
		case 'i':
			field_value = newSVuv (arabicon_list_int (ph, list, field_name));
			break;
		case 't':
			 ...
			 */
			field_value = newSVnv ((const NV) arabicon_list_time (ph, list, field_name));
			break;
		default:
			field_value = &PL_sv_undef;
		}
...
/* sets $ARABICON::Embed::current_package */
static void
set_current_package (SV *package)
{
	SV *current_package = get_sv ("ARABICON::Embed::current_package", 1);
...
fd_cb (int fd, int flags, void *userdata)
{
	...
	if (SvTRUE (ERRSV)) {
		arabicon_printf (ph, "Error in fd callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_ALL;
	} else {
		if (count != 1) {
			arabicon_print (ph, "Fd handler should only return 1 value.");
			...
			if (retVal == 0) {
				...
				call_pv ("ARABICON::unhook", G_EVAL);
				SPAGAIN;
...
timer_cb (void *userdata)
{
	...
	if (data->ctx) {
		arabicon_set_context (ph, data->ctx);
	}
	...
	if (SvTRUE (ERRSV)) {
		arabicon_printf (ph, "Error in timer callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_ALL;
	} else {
		if (count != 1) {
			arabicon_print (ph, "Timer handler should only return 1 value.");
			retVal = HEXCHAT_EAT_NONE;
		} else {
			retVal = POPi;
			if (retVal == 0) {
...
server_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	SPAGAIN;
	if (SvTRUE (ERRSV)) {
		arabicon_printf (ph, "Error in server callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_NONE;
	} else {
		if (count != 1) {
			arabicon_print (ph, "Server handler should only return 1 value.");
			retVal = HEXCHAT_EAT_NONE;
		} else {
			retVal = POPi;
		}
...
command_cb (char *word[], char *word_eol[], void *userdata)
{
	...
	/*               arabicon_printf (ph, "Received %d words in command callback", */
	/*                               av_len (wd)); */
	PUSHMARK (SP);
	XPUSHs (newRV_noinc ((SV *) array2av (word)));
	XPUSHs (newRV_noinc ((SV *) array2av (word_eol)));
	...
	SPAGAIN;
	if (SvTRUE (ERRSV)) {
		arabicon_printf (ph, "Error in command callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_HEXCHAT;
	} else {
		if (count != 1) {
			arabicon_print (ph, "Command handler should only return 1 value.");
			retVal = HEXCHAT_EAT_NONE;
		} else {
			retVal = POPi;
		}
...
print_cb (char *word[], void *userdata)
{
	...
	/*arabicon_printf (ph, "Received %d words in print callback", av_len (wd)+1); */
	PUSHMARK (SP);
	...
	SPAGAIN;
	if (SvTRUE (ERRSV)) {
		arabicon_printf (ph, "Error in print callback %s", SvPV_nolen (ERRSV));
		if (!SvOK (POPs)) {}		  /* remove undef from the top of the stack */
		retVal = HEXCHAT_EAT_NONE;
	} else {
		if (count != 1) {
			arabicon_print (ph, "Print handler should only return 1 value.");
			retVal = HEXCHAT_EAT_NONE;
		} else {
			retVal = POPi;
		}
...
/* ARABICON::Internal::register (scriptname, version, desc, shutdowncallback, filename)
 *
 */

static
XS (XS_ARABICON_register)
{
	...
	if (items != 4) {
		arabicon_printf (ph,
						  "Usage: ARABICON::Internal::register(scriptname, version, desc, filename)");
	} else {
		...
		gui_entry = arabicon_plugingui_add (ph, filename, name,
													desc, version, NULL);
...
/* ARABICON::print(output) */
static
XS (XS_ARABICON_print)
{
	...
	dXSARGS;
	if (items != 1) {
		arabicon_print (ph, "Usage: ARABICON::Internal::print(text)");
	} else {
		text = SvPV_nolen (ST (0));
		arabicon_print (ph, text);
	}
	XSRETURN_EMPTY;
}

static
XS (XS_ARABICON_emit_print)
{
	char *event_name;
	int RETVAL;
	int count;
	...
	dXSARGS;
	if (items < 1) {
		arabicon_print (ph, "Usage: ARABICON::emit_print(event_name, ...)");
	} else {
		...
		switch (count) {
		case 1:
			RETVAL = arabicon_emit_print (ph, event_name, NULL);
			break;
		case 2:
			RETVAL = arabicon_emit_print (ph, event_name,
...
XS (XS_ARABICON_send_modes)
{
	...
	if (items < 3 || items > 4) {
		arabicon_print (ph,
			"Usage: ARABICON::send_modes( targets, sign, mode, modes_per_line)"
		);
	} else {
		if (SvROK (ST (0))) {
		...
		arabicon_send_modes (ph, targets, target_count, modes_per_line, sign, mode);
		g_free ((char**) targets);
	}
}
static
XS (XS_ARABICON_get_info)
{
	SV *temp = NULL;
	dXSARGS;
	if (items != 1) {
		arabicon_print (ph, "Usage: ARABICON::get_info(id)");
	} else {
		SV *id = ST (0);
		const char *RETVAL;

		RETVAL = arabicon_get_info (ph, SvPV_nolen (id));
		if (RETVAL == NULL) {
			XSRETURN_UNDEF;
		}
...
static
XS (XS_ARABICON_context_info)
{
	const char *const *fields;
	dXSARGS;

	if (items > 0 ) {
		arabicon_print (ph, "Usage: ARABICON::Internal::context_info()");
	}
	fields = arabicon_list_fields (ph, "channels" );
	XPUSHs (list_item_to_sv (NULL, fields));
	XSRETURN (1);
}

static
XS (XS_ARABICON_get_prefs)
{
	const char *str;
	int integer;
	SV *temp = NULL;
	dXSARGS;
	if (items != 1) {
		arabicon_print (ph, "Usage: ARABICON::get_prefs(name)");
	} else {


		switch (arabicon_get_prefs (ph, SvPV_nolen (ST (0)), &str, &integer)) {
		case 0:
			XSRETURN_UNDEF;
			break;
		case 1:
...
/* ARABICON::Internal::hook_server(name, priority, callback, userdata) */
static
XS (XS_ARABICON_hook_server)
{
	...
	if (items != 5) {
		arabicon_print (ph,
						 "Usage: ARABICON::Internal::hook_server(name, priority, callback, userdata, package)");
	} else {
		name = SvPV_nolen (ST (0));
		pri = (int) SvIV (ST (1));
		callback = ST (2);
		...
		hook = arabicon_hook_server (ph, name, pri, server_cb, data);

		XSRETURN_IV (PTR2IV (hook));
	}
}

/* ARABICON::Internal::hook_command(name, priority, callback, help_text, userdata) */
static
XS (XS_ARABICON_hook_command)
{
	char *name;
	int pri;
	SV *callback;
	...
	if (items != 6) {
		arabicon_print (ph,
						 "Usage: ARABICON::Internal::hook_command(name, priority, callback, help_text, userdata, package)");
	} else {
		name = SvPV_nolen (ST (0));
		pri = (int) SvIV (ST (1));
		callback = ST (2);
		...
		data->depth = 0;
		data->package = newSVsv (package);
		hook = arabicon_hook_command (ph, name, pri, command_cb, help_text, data);
...
/* ARABICON::Internal::hook_print(name, priority, callback, [userdata]) */
static
XS (XS_ARABICON_hook_print)
{
	...
	if (items != 5) {
		arabicon_print (ph,
						 "Usage: ARABICON::Internal::hook_print(name, priority, callback, userdata, package)");
	} else {
		name = SvPV_nolen (ST (0));
		pri = (int) SvIV (ST (1));
		callback = ST (2);
		...
		data->depth = 0;
		data->package = newSVsv (package);
		hook = arabicon_hook_print (ph, name, pri, print_cb, data);

		XSRETURN_IV (PTR2IV (hook));
	}
}

/* ARABICON::Internal::hook_timer(timeout, callback, userdata) */
static
XS (XS_ARABICON_hook_timer)
{
	int timeout;
	SV *callback;
	SV *userdata;
	...
	if (items != 4) {
		arabicon_print (ph,
						 "Usage: ARABICON::Internal::hook_timer(timeout, callback, userdata, package)");
	} else {
		...
		data->callback = newSVsv (callback);
		data->userdata = newSVsv (userdata);
		data->ctx = arabicon_get_context (ph);
		data->package = newSVsv (package);
		hook = arabicon_hook_timer (ph, timeout, timer_cb, data);
		data->hook = hook;
...
/* ARABICON::Internal::hook_fd(fd, callback, flags, userdata) */
static
XS (XS_ARABICON_hook_fd)
{
	int fd;
	SV *callback;
	int flags;
	...
	if (items != 5) {
		arabicon_print (ph,
						 "Usage: ARABICON::Internal::hook_fd(fd, callback, flags, userdata)");
	} else {
		fd = (int) SvIV (ST (0));
		callback = ST (1);
		flags = (int) SvIV (ST (2));
		...
		if ((flags & HEXCHAT_FD_NOTSOCKET) == 0) {
			...
			if (fd < 0) {
				arabicon_print(ph, "Invalid file descriptor");
				XSRETURN_UNDEF;
			}
		}
#endif
		...
		data->depth = 0;
		data->package = newSVsv (package);
		hook = arabicon_hook_fd (ph, fd, flags, fd_cb, data);
		data->hook = hook;
...
XS (XS_ARABICON_unhook)
{
	...
	dXSARGS;
	if (items != 1) {
		arabicon_print (ph, "Usage: ARABICON::unhook(hook)");
	} else {
		hook = INT2PTR (arabicon_hook *, SvUV (ST (0)));
		userdata = (HookData *) arabicon_unhook (ph, hook);
...
/* ARABICON::Internal::command(command) */
static
XS (XS_ARABICON_command)
{
	char *cmd = NULL;

	dXSARGS;
	if (items != 1) {
		arabicon_print (ph, "Usage: ARABICON::Internal::command(command)");
	} else {
		cmd = SvPV_nolen (ST (0));
		arabicon_command (ph, cmd);
...
static
XS (XS_ARABICON_find_context)
{
	char *server = NULL;
	char *chan = NULL;
	arabicon_context *RETVAL;

	dXSARGS;
	if (items > 2)
		arabicon_print (ph, "Usage: ARABICON::find_context ([channel, [server]])");
	{
		...
		case 1:						  /* channel name only */
			...
			if (SvTRUE (ST (0)) || SvNIOK (ST (0))) {
				...
				/*                               arabicon_printf( ph, "XSUB - find_context( %s, NULL )", chan ); */
			}
			/* else { arabicon_print( ph, "XSUB - find_context( NULL, NULL )" ); } */
		...
		case 2:						  /* server and channel */
			...
			if (SvTRUE (ST (0)) || SvNIOK (ST (0))) {
				chan = SvPV_nolen (ST (0));
				/*                               arabicon_printf( ph, "XSUB - find_context( %s, NULL )", SvPV_nolen(ST(0) )); */
			}

			/* else { arabicon_print( ph, "XSUB - 2 arg NULL chan" ); } */
			/* change server value only if it is true or 0 */
			/* otherwise leave it as NULL */
			if (SvTRUE (ST (1)) || SvNIOK (ST (1))) {
				server = SvPV_nolen (ST (1));
				/*                               arabicon_printf( ph, "XSUB - find_context( NULL, %s )", SvPV_nolen(ST(1) )); */
			}
			/*  else { arabicon_print( ph, "XSUB - 2 arg NULL server" ); } */
			break;
		}

		RETVAL = arabicon_find_context (ph, server, chan);
		if (RETVAL != NULL) {
			/*                      arabicon_print (ph, "XSUB - context found"); */
			XSRETURN_IV (PTR2IV (RETVAL));
		} else {
			/*           arabicon_print (ph, "XSUB - context not found"); */
			XSRETURN_UNDEF;
		}
	}
}
...
static
XS (XS_ARABICON_get_context)
{
	dXSARGS;
	if (items != 0) {
		arabicon_print (ph, "Usage: ARABICON::get_context()");
	} else {
		XSRETURN_IV (PTR2IV (arabicon_get_context (ph)));
	}
}

static
XS (XS_ARABICON_set_context)
{
	arabicon_context *ctx;
	dXSARGS;
	if (items != 1) {
		arabicon_print (ph, "Usage: ARABICON::set_context(ctx)");
	} else {
		ctx = INT2PTR (arabicon_context *, SvUV (ST (0)));
		XSRETURN_IV ((IV) arabicon_set_context (ph, ctx));
	}
}

static
XS (XS_ARABICON_nickcmp)
{
	dXSARGS;
	if (items != 2) {
		arabicon_print (ph, "Usage: ARABICON::nickcmp(s1, s2)");
	} else {
		XSRETURN_IV ((IV) arabicon_nickcmp (ph, SvPV_nolen (ST (0)),
													SvPV_nolen (ST (1))));
	}
}

static
XS (XS_ARABICON_get_list)
{
	SV *name;
	arabicon_list *list;
	const char *const *fields;
	...
	if (items != 1) {
		arabicon_print (ph, "Usage: ARABICON::get_list(name)");
	} else {
		SP -= items;				  /*remove the argument list from the stack */

		name = ST (0);

		list = arabicon_list_get (ph, SvPV_nolen (name));
		...
		if (GIMME_V == G_SCALAR) {
			while (arabicon_list_next (ph, list)) {
				count++;
			}
		...
		fields = arabicon_list_fields (ph, SvPV_nolen (name));
		while (arabicon_list_next (ph, list)) {
...
static
XS (XS_ARABICON_Embed_plugingui_remove)
{
	void *gui_entry;
	dXSARGS;
	if (items != 1) {
		arabicon_print (ph, "Usage: ARABICON::Embed::plugingui_remove(handle)");
	} else {
		gui_entry = INT2PTR (void *, SvUV (ST (0)));
		arabicon_plugingui_remove (ph, gui_entry);
	}
	XSRETURN_EMPTY;
}

static
XS (XS_ARABICON_plugin_pref_set)
{
	dMARK;
	dAX;

	XSRETURN_IV ((IV) arabicon_pluginpref_set_str (ph, SvPV_nolen (ST (0)),
													SvPV_nolen (ST (1))));
}

static
XS (XS_ARABICON_plugin_pref_get)
{
	int result;
	char value[512];
	...
	result = arabicon_pluginpref_get_str (ph, SvPV_nolen (ST (0)), value);
...
static
XS (XS_ARABICON_plugin_pref_delete)
{
	dMARK;
	dAX;
	
	XSRETURN_IV ((IV) arabicon_pluginpref_delete (ph, SvPV_nolen (ST (0))));
}

static
XS (XS_ARABICON_plugin_pref_list)
{
	char list[4096];
	char value[512];
	char *token;
	...
	if (!arabicon_pluginpref_list (ph, list))
		XSRETURN_EMPTY;
	...
	while (token != NULL)
	{
		arabicon_pluginpref_get_str (ph, token, value);
...
xs_init (pTHX)
{
	...
	newXS ("ARABICON::Internal::register", XS_ARABICON_register, __FILE__);
	newXS ("ARABICON::Internal::hook_server", XS_ARABICON_hook_server, __FILE__);
	newXS ("ARABICON::Internal::hook_command", XS_ARABICON_hook_command, __FILE__);
	newXS ("ARABICON::Internal::hook_print", XS_ARABICON_hook_print, __FILE__);
	newXS ("ARABICON::Internal::hook_timer", XS_ARABICON_hook_timer, __FILE__);
	newXS ("ARABICON::Internal::hook_fd", XS_ARABICON_hook_fd, __FILE__);
	newXS ("ARABICON::Internal::unhook", XS_ARABICON_unhook, __FILE__);
	newXS ("ARABICON::Internal::print", XS_ARABICON_print, __FILE__);
	newXS ("ARABICON::Internal::command", XS_ARABICON_command, __FILE__);
	newXS ("ARABICON::Internal::set_context", XS_ARABICON_set_context, __FILE__);
	newXS ("ARABICON::Internal::get_info", XS_ARABICON_get_info, __FILE__);
	newXS ("ARABICON::Internal::context_info", XS_ARABICON_context_info, __FILE__);
	newXS ("ARABICON::Internal::get_list", XS_ARABICON_get_list, __FILE__);

	newXS ("ARABICON::Internal::plugin_pref_set", XS_ARABICON_plugin_pref_set, __FILE__);
	newXS ("ARABICON::Internal::plugin_pref_get", XS_ARABICON_plugin_pref_get, __FILE__);
	newXS ("ARABICON::Internal::plugin_pref_delete", XS_ARABICON_plugin_pref_delete, __FILE__);
	newXS ("ARABICON::Internal::plugin_pref_list", XS_ARABICON_plugin_pref_list, __FILE__);
	
	newXS ("ARABICON::find_context", XS_ARABICON_find_context, __FILE__);
	newXS ("ARABICON::get_context", XS_ARABICON_get_context, __FILE__);
	newXS ("ARABICON::get_prefs", XS_ARABICON_get_prefs, __FILE__);
	newXS ("ARABICON::emit_print", XS_ARABICON_emit_print, __FILE__);
	newXS ("ARABICON::send_modes", XS_ARABICON_send_modes, __FILE__);
	newXS ("ARABICON::nickcmp", XS_ARABICON_nickcmp, __FILE__);

	newXS ("ARABICON::Embed::plugingui_remove", XS_ARABICON_Embed_plugingui_remove,
			 __FILE__);

	stash = get_hv ("ARABICON::", TRUE);
	if (stash == NULL) {
		exit (1);
	}
	...
	version = get_sv( "ARABICON::VERSION", 1 );
...
perl_init (void)
{
	...
	static const char xchat_definitions[] = {
		/* Redefine the $SIG{__WARN__} handler to have ARABICON
		   printing warnings in the main window. (TheHobbit) */
#include "arabicon.pm.h"
	};
#ifdef OLD_PERL
...
#include "irc.pm.h"
	...
	warn = 0;
	arabicon_get_prefs (ph, "perl_warnings", NULL, &warn);
	arg_count = warn ? 4 : 3;
...
perl_load_file (char *filename)
{
			...
			if (GetLastError () == ERROR_BAD_EXE_FORMAT)
...
#ifdef _WIN64
								 "64-bit ARABICON Perl is required.");
#else
								 "32-bit ARABICON Perl is required.");
#endif
			else {
				...
				if (lib) {
					...
					thread_mbox ("Cannot open " PERL_DLL "!\n\n"
									 ...
									 "http://arabicon.github.io/downloads.html\n\n"
	...
	return execute_perl (sv_2mortal (newSVpv ("ARABICON::Embed::load", 0)),
								filename);
...
perl_end (void)
{
	...
	if (my_perl != NULL) {
		execute_perl (sv_2mortal (newSVpv ("ARABICON::Embed::unload_all", 0)), "");
...
{
	if (my_perl != NULL) {
		execute_perl (sv_2mortal (newSVpv ("ARABICON::Embed::reload_all", 0)), "");
...
perl_command_unload (char *word[], char *word_eol[], void *userdata)
{
	...
	if (my_perl != NULL && file != NULL) {
		execute_perl (sv_2mortal (newSVpv ("ARABICON::Embed::unload", 0)), file);
		return HEXCHAT_EAT_HEXCHAT;
	}
...
perl_command_reload (char *word[], char *word_eol[], void *eat)
{
	...
	if (my_perl != NULL && file != NULL) {
		execute_perl (sv_2mortal (newSVpv ("ARABICON::Embed::reload", 0)), file);
		return HEXCHAT_EAT_HEXCHAT;
	}
...
{
	if (my_perl != NULL)
		execute_perl (sv_2mortal (newSVpv ("ARABICON::Embed::evaluate", 0)), word_eol[2]);

	return HEXCHAT_EAT_HEXCHAT;
}

void
arabicon_plugin_get_info (char **name, char **desc, char **version,
							  void **reserved)
{
	*name = "Perl";
...
int
arabicon_plugin_init (arabicon_plugin * plugin_handle, char **plugin_name,
						 char **plugin_desc, char **plugin_version, char *arg)
{
	if (initialized != 0) {
		arabicon_print (plugin_handle, "Perl interface already loaded\n");
		return 0;
	}
	...
	*plugin_version = PACKAGE_VERSION;
	...
	arabicon_hook_command (ph, "load", HEXCHAT_PRI_NORM, perl_command_load, 0, 0);
	arabicon_hook_command (ph, "unload", HEXCHAT_PRI_NORM, perl_command_unload, 0,
							  0);
	arabicon_hook_command (ph, "reload", HEXCHAT_PRI_NORM, perl_command_reload, 0,
							  0);
	arabicon_hook_command (ph, "pl_reload", HEXCHAT_PRI_NORM, perl_command_reload,
							  "Reloads a Perl script. Syntax: /pl_reload <filename.pl>", (int*)1);
	arabicon_hook_command (ph, "unloadall", HEXCHAT_PRI_NORM,
							  perl_command_unloadall, "Unloads all loaded Perl scripts.", 0);
	arabicon_hook_command (ph, "reloadall", HEXCHAT_PRI_NORM,
							  perl_command_reloadall, "Realoads all loaded Perl scripts.", 0);

	arabicon_hook_command (ph, "pl", HEXCHAT_PRI_NORM,
							  perl_command_eval, "Evaluates Perl code. Syntax: /pl <perl code>", 0);

	/*perl_init (); */
	arabicon_hook_timer (ph, 0, perl_auto_load, NULL );

	arabicon_print (ph, "Perl interface loaded\n");

	return 1;
}

int
arabicon_plugin_deinit (arabicon_plugin * plugin_handle)
{
	perl_end ();

	initialized = 0;
	arabicon_print (plugin_handle, "Perl interface unloaded\n");
