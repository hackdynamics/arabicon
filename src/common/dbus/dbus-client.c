/* dbus-client.c - ARABICON command-line options for D-Bus
...
#define DBUS_REMOTE_PATH "/org/arabicon/Remote"
#define DBUS_REMOTE_INTERFACE "org.arabicon.plugin"
 ...
 */
{
	...
	/* Checks if ARABICON is already running */
	...
	if (!ret)
	{
		...
		arabicon_running = FALSE;
	}
	else
	{
		GVariant *child = g_variant_get_child_value (ret, 0);
		arabicon_running = g_variant_get_boolean (child);
		g_variant_unref (ret);
	...
	if (!remote_object)
	{
		write_error("Failed to connect to ARABICON", &error);
		g_object_unref (connection);
		exit (0);
	}
