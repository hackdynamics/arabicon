...
#include "util.h"
#include "fe.h"
#include "cfgfiles.h"			  /* arabicon_fopen_file() */
...
random_line (char *file_name)
{
	...
	fh = arabicon_fopen_file (file_name, "r", 0);
	if (!fh)
	{
	 nofile:
...
{
	/* this is pretty much the same as in arabicon_exit() */
	save_config ();
...
load_perform_file (session *sess, char *file)
{
	...
	fp = arabicon_fopen_file (file, "r", 0);		/* load files from config dir */
	if (!fp)
		return FALSE;
...
cmd_tray (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (!word[3][0])
	{
		fe_tray_set_file (NULL);	/* default ARABICON icon */
		return TRUE;
	}
...
const struct commands xc_cmds[] = {
...
#endif
#if 0
	{"EXPORTCONF", cmd_exportconf, 0, 0, 1, N_("EXPORTCONF, exports ARABICON settings")},
#endif
	...
	{"MENU", cmd_menu, 0, 0, 1, "MENU [-eX] [-i<ICONFILE>] [-k<mod>,<key>] [-m] [-pX] [-r<X,group>] [-tX] {ADD|DEL} <path> [command] [unselect command]\n"
										 "       See http://arabicon.readthedocs.org/en/latest/plugins.html#controlling-the-gui for more details."},
	{"MHOP", cmd_mhop, 1, 1, 1,
...
#endif
	{"RECV", cmd_recv, 1, 0, 1, N_("RECV <text>, send raw data to ARABICON, as if it was received from the IRC server")},
