...
#include "text.h"
#define PLUGIN_C
typedef struct session arabicon_context;
...
struct _arabicon_hook
{
	arabicon_plugin *pl;	/* the plugin to which it belongs */
	char *name;			/* "xdcc" */
...
typedef int (arabicon_cmd_cb) (char *word[], char *word_eol[], void *user_data);
typedef int (arabicon_serv_cb) (char *word[], char *word_eol[], void *user_data);
typedef int (arabicon_print_cb) (char *word[], void *user_data);
typedef int (arabicon_serv_attrs_cb) (char *word[], char *word_eol[], arabicon_event_attrs *attrs, void *user_data);
typedef int (arabicon_print_attrs_cb) (char *word[], arabicon_event_attrs *attrs, void *user_data);
typedef int (arabicon_fd_cb) (int fd, int flags, void *user_data);
typedef int (arabicon_timer_cb) (void *user_data);
typedef int (arabicon_init_func) (arabicon_plugin *, char **, char **, char **, char *);
typedef int (arabicon_deinit_func) (arabicon_plugin *);
...
static int
plugin_free (arabicon_plugin *pl, int do_deinit, int allow_refuse)
{
	GSList *list, *next;
	arabicon_hook *hook;
	arabicon_deinit_func *deinit_func;

	/* fake plugin added by arabicon_plugingui_add() */
	if (pl->fake)
		goto xit;
...
static arabicon_plugin *
plugin_list_add (arabicon_context *ctx, char *filename, const char *name,
					  const char *desc, const char *version, void *handle,
					  void *deinit_func, int fake, int free_strings)
{
	arabicon_plugin *pl;

	pl = g_new (arabicon_plugin, 1);
	pl->handle = handle;
...
#ifndef WIN32
static void *
arabicon_dummy (arabicon_plugin *ph)
{
	return NULL;
}
...
static int
arabicon_read_fd (arabicon_plugin *ph, GIOChannel *source, char *buf, int *len)
{
	GError *error = NULL;
				...
				void *deinit_func, char *arg, int fake)
{
	...
	if (!fake)
	{
		...
		pl->arabicon_hook_command = arabicon_hook_command;
		pl->arabicon_hook_server = arabicon_hook_server;
		pl->arabicon_hook_print = arabicon_hook_print;
		pl->arabicon_hook_timer = arabicon_hook_timer;
		pl->arabicon_hook_fd = arabicon_hook_fd;
		pl->arabicon_unhook = arabicon_unhook;
		pl->arabicon_print = arabicon_print;
		pl->arabicon_printf = arabicon_printf;
		pl->arabicon_command = arabicon_command;
		pl->arabicon_commandf = arabicon_commandf;
		pl->arabicon_nickcmp = arabicon_nickcmp;
		pl->arabicon_set_context = arabicon_set_context;
		pl->arabicon_find_context = arabicon_find_context;
		pl->arabicon_get_context = arabicon_get_context;
		pl->arabicon_get_info = arabicon_get_info;
		pl->arabicon_get_prefs = arabicon_get_prefs;
		pl->arabicon_list_get = arabicon_list_get;
		pl->arabicon_list_free = arabicon_list_free;
		pl->arabicon_list_fields = arabicon_list_fields;
		pl->arabicon_list_str = arabicon_list_str;
		pl->arabicon_list_next = arabicon_list_next;
		pl->arabicon_list_int = arabicon_list_int;
		pl->arabicon_plugingui_add = arabicon_plugingui_add;
		pl->arabicon_plugingui_remove = arabicon_plugingui_remove;
		pl->arabicon_emit_print = arabicon_emit_print;
#ifdef WIN32
		pl->arabicon_read_fd = (void *) arabicon_read_fd;
#else
		pl->arabicon_read_fd = arabicon_dummy;
#endif
		pl->arabicon_list_time = arabicon_list_time;
		pl->arabicon_gettext = arabicon_gettext;
		pl->arabicon_send_modes = arabicon_send_modes;
		pl->arabicon_strip = arabicon_strip;
		pl->arabicon_free = arabicon_free;
		pl->arabicon_pluginpref_set_str = arabicon_pluginpref_set_str;
		pl->arabicon_pluginpref_get_str = arabicon_pluginpref_get_str;
		pl->arabicon_pluginpref_set_int = arabicon_pluginpref_set_int;
		pl->arabicon_pluginpref_get_int = arabicon_pluginpref_get_int;
		pl->arabicon_pluginpref_delete = arabicon_pluginpref_delete;
		pl->arabicon_pluginpref_list = arabicon_pluginpref_list;
		pl->arabicon_hook_server_attrs = arabicon_hook_server_attrs;
		pl->arabicon_hook_print_attrs = arabicon_hook_print_attrs;
		pl->arabicon_emit_print_attrs = arabicon_emit_print_attrs;
		pl->arabicon_event_attrs_create = arabicon_event_attrs_create;
		pl->arabicon_event_attrs_free = arabicon_event_attrs_free;

		/* run arabicon_plugin_init, if it returns 0, close the plugin */
		if (((arabicon_init_func *)init_func) (pl, &pl->name, &pl->desc, &pl->version, arg) == 0)
		{
...
plugin_load (session *sess, char *filename, char *arg)
{
	...
	arabicon_init_func *init_func;
	arabicon_deinit_func *deinit_func;

	if (handle == NULL)
		return (char *)g_module_error ();

	/* find the init routine arabicon_plugin_init */
	if (!g_module_symbol (handle, "arabicon_plugin_init", (gpointer *)&init_func))
	{
		g_module_close (handle);
		return _("No arabicon_plugin_init symbol; is this really a ARABICON plugin?");
	}

	/* find the plugin's deinit routine, if any */
	if (!g_module_symbol (handle, "arabicon_plugin_deinit", (gpointer *)&deinit_func))
		deinit_func = NULL;
...
plugin_hook_run (session *sess, char *name, char *word[], char *word_eol[],
				 arabicon_event_attrs *attrs, int type)
{
	GSList *list, *next;
	arabicon_hook *hook;
	int ret, eat = 0;
	...
	while (1)
	{
		...
		{
		case HOOK_COMMAND:
			ret = ((arabicon_cmd_cb *)hook->callback) (word, word_eol, hook->userdata);
			break;
		case HOOK_PRINT_ATTRS:
			ret = ((arabicon_print_attrs_cb *)hook->callback) (word, attrs, hook->userdata);
			break;
		case HOOK_SERVER:
			ret = ((arabicon_serv_cb *)hook->callback) (word, word_eol, hook->userdata);
			break;
		case HOOK_SERVER_ATTRS:
			ret = ((arabicon_serv_attrs_cb *)hook->callback) (word, word_eol, attrs, hook->userdata);
			break;
		default: /*case HOOK_PRINT:*/
			ret = ((arabicon_print_cb *)hook->callback) (word, hook->userdata);
			break;
		}
...
arabicon_event_attrs_create (arabicon_plugin *ph)
{
	return g_new0 (arabicon_event_attrs, 1);
}

void
arabicon_event_attrs_free (arabicon_plugin *ph, arabicon_event_attrs *attrs)
{
	g_free (attrs);
}
...
static int
plugin_timeout_cb (arabicon_hook *hook)
{
	int ret;
	...
	ret = ((arabicon_timer_cb *)hook->callback) (hook->userdata);
...
static void
plugin_insert_hook (arabicon_hook *new_hook)
{
	GSList *list;
	arabicon_hook *hook;
	int new_hook_type;
...
static gboolean
plugin_fd_cb (GIOChannel *source, GIOCondition condition, arabicon_hook *hook)
{
	int flags = 0, ret;
	typedef int (arabicon_fd_cb2) (int fd, int flags, void *user_data, GIOChannel *);
	...
	ret = ((arabicon_fd_cb2 *)hook->callback) (hook->pri, flags, hook->userdata, source);
...
static arabicon_hook *
plugin_add_hook (arabicon_plugin *pl, int type, int pri, const char *name,
					  const  char *help_text, void *callb, int timeout, void *userdata)
{
	arabicon_hook *hook;

	hook = g_new0 (arabicon_hook, 1);
	hook->type = type;
	hook->pri = pri;
...
void *
arabicon_unhook (arabicon_plugin *ph, arabicon_hook *hook)
{
	/* perl.c trips this */
	if (!g_slist_find (hook_list, hook) || hook->type == HOOK_DELETED)
		return NULL;
...
arabicon_hook *
arabicon_hook_command (arabicon_plugin *ph, const char *name, int pri,
						  arabicon_cmd_cb *callb, const char *help_text, void *userdata)
{
...
arabicon_hook *
arabicon_hook_server (arabicon_plugin *ph, const char *name, int pri,
						 arabicon_serv_cb *callb, void *userdata)
{
	return plugin_add_hook (ph, HOOK_SERVER, pri, name, 0, callb, 0, userdata);
}

arabicon_hook *
arabicon_hook_server_attrs (arabicon_plugin *ph, const char *name, int pri,
						   arabicon_serv_attrs_cb *callb, void *userdata)
{
...
arabicon_hook *
arabicon_hook_print (arabicon_plugin *ph, const char *name, int pri,
						arabicon_print_cb *callb, void *userdata)
{
	return plugin_add_hook (ph, HOOK_PRINT, pri, name, 0, callb, 0, userdata);
}

arabicon_hook *
arabicon_hook_print_attrs (arabicon_plugin *ph, const char *name, int pri,
						  arabicon_print_attrs_cb *callb, void *userdata)
{
...
arabicon_hook *
arabicon_hook_timer (arabicon_plugin *ph, int timeout, arabicon_timer_cb *callb,
					   void *userdata)
{
...
arabicon_hook *
arabicon_hook_fd (arabicon_plugin *ph, int fd, int flags,
					arabicon_fd_cb *callb, void *userdata)
{
	arabicon_hook *hook;
...
void
arabicon_print (arabicon_plugin *ph, const char *text)
{
	if (!is_session (ph->context))
	{
		DEBUG(PrintTextf(0, "%s\tarabicon_print called without a valid context.\n", ph->name));
		return;
	}
...
void
arabicon_printf (arabicon_plugin *ph, const char *format, ...)
{
	va_list args;
	char *buf;
	...
	arabicon_print (ph, buf);
	g_free (buf);
}

void
arabicon_command (arabicon_plugin *ph, const char *command)
{
	char *command_utf8;

	if (!is_session (ph->context))
	{
		DEBUG(PrintTextf(0, "%s\tarabicon_command called without a valid context.\n", ph->name));
		return;
	}
...
void
arabicon_commandf (arabicon_plugin *ph, const char *format, ...)
{
	va_list args;
	char *buf;
	...
	arabicon_command (ph, buf);
	g_free (buf);
}

int
arabicon_nickcmp (arabicon_plugin *ph, const char *s1, const char *s2)
{
...
int
arabicon_set_context (arabicon_plugin *ph, arabicon_context *context)
{
...
arabicon_context *
arabicon_find_context (arabicon_plugin *ph, const char *servname, const char *channel)
{
	return plugin_find_context (servname, channel, ph->context->server);
}

const char *
arabicon_get_info (arabicon_plugin *ph, const char *id)
{
	session *sess;
	guint32 hash;
	...
	if (!is_session (sess))
	{
		DEBUG(PrintTextf(0, "%s\tarabicon_get_info called without a valid context.\n", ph->name));
		return NULL;
	}
...
int
arabicon_get_prefs (arabicon_plugin *ph, const char *name, const char **string, int *integer)
{
	int i = 0;
...
arabicon_list *
arabicon_list_get (arabicon_plugin *ph, const char *name)
{
	arabicon_list *list;

	list = g_new0 (arabicon_list, 1);
...
void
arabicon_list_free (arabicon_plugin *ph, arabicon_list *xlist)
{
...
int
arabicon_list_next (arabicon_plugin *ph, arabicon_list *xlist)
{
	if (xlist->next == NULL)
		return 0;
...
const char * const *
arabicon_list_fields (arabicon_plugin *ph, const char *name)
{
...
time_t
arabicon_list_time (arabicon_plugin *ph, arabicon_list *xlist, const char *name)
{
...
const char *
arabicon_list_str (arabicon_plugin *ph, arabicon_list *xlist, const char *name)
{
	guint32 hash = str_hash (name);
...
int
arabicon_list_int (arabicon_plugin *ph, arabicon_list *xlist, const char *name)
{
	guint32 hash = str_hash (name);
	gpointer data = ph->context;
...
void *
arabicon_plugingui_add (arabicon_plugin *ph, const char *filename,
...
void
arabicon_plugingui_remove (arabicon_plugin *ph, void *handle)
{
#ifdef USE_PLUGIN
...
int
arabicon_emit_print (arabicon_plugin *ph, const char *event_name, ...)
{
	va_list args;
...
char *
arabicon_gettext (arabicon_plugin *ph, const char *msgid)
{
	/* so that plugins can use ARABICON's internal gettext strings. */
	/* e.g. The EXEC plugin uses this on Windows. */
	return _(msgid);
}

void
arabicon_send_modes (arabicon_plugin *ph, const char **targets, int ntargets, int modes_per_line, char sign, char mode)
{
...
char *
arabicon_strip (arabicon_plugin *ph, const char *str, int len, int flags)
{
	return strip_color ((char *)str, len, flags);
}

void
arabicon_free (arabicon_plugin *ph, void *ptr)
{
	g_free (ptr);
}

static int
arabicon_pluginpref_set_str_real (arabicon_plugin *pl, const char *var, const char *value, int mode) /* mode: 0 = delete, 1 = save */
{
	FILE *fpIn;
	int fhOut;
	int prevSetting;
	...
	fhOut = arabicon_open_file (confname_tmp, O_TRUNC | O_WRONLY | O_CREAT, 0600, XOF_DOMODE);
	fpIn = arabicon_fopen_file (confname, "r", 0);
...
int
arabicon_pluginpref_set_str (arabicon_plugin *pl, const char *var, const char *value)
{
	return arabicon_pluginpref_set_str_real (pl, var, value, 1);
}

static int
arabicon_pluginpref_get_str_real (arabicon_plugin *pl, const char *var, char *dest, int dest_len)
{
...
int
arabicon_pluginpref_get_str (arabicon_plugin *pl, const char *var, char *dest)
{
	/* All users of this must ensure dest is >= 512... */
	return arabicon_pluginpref_get_str_real (pl, var, dest, 512);
}

int
arabicon_pluginpref_set_int (arabicon_plugin *pl, const char *var, int value)
{
	char buffer[12];

	g_snprintf (buffer, sizeof (buffer), "%d", value);
	return arabicon_pluginpref_set_str_real (pl, var, buffer, 1);
}

int
arabicon_pluginpref_get_int (arabicon_plugin *pl, const char *var)
{
	char buffer[12];

	if (arabicon_pluginpref_get_str_real (pl, var, buffer, sizeof(buffer)))
	{
		int ret = atoi (buffer);
...
int
arabicon_pluginpref_delete (arabicon_plugin *pl, const char *var)
{
	return arabicon_pluginpref_set_str_real (pl, var, 0, 0);
}

int
arabicon_pluginpref_list (arabicon_plugin *pl, char* dest)
{
	FILE *fpIn;
	char confname[64];
