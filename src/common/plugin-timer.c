...
timer_del_ref (int ref, int quiet)
{
	...
	while (list)
	{
		...
		if (tim->ref == ref)
		{
			timer_del (tim);
			if (!quiet)
				arabicon_printf (ph, _("Timer %d deleted.\n"), ref);
			return;
		}
		list = list->next;
	}
	if (!quiet)
		arabicon_print (ph, _("No such ref number found.\n"));
}

static int
timeout_cb (timer *tim)
{
	if (arabicon_set_context (ph, tim->context))
	{
		arabicon_command (ph, tim->command);
...
timer_add (int ref, int timeout, int repeat, char *command)
{
	...
	tim->context = arabicon_get_context (ph);
	tim->forever = FALSE;

	if (repeat == 0)
		tim->forever = TRUE;

	tim->hook = arabicon_hook_timer (ph, timeout, (void *)timeout_cb, tim);
...
timer_showlist (void)
{
	...
	if (timer_list == NULL)
	{
		arabicon_print (ph, _("No timers installed.\n"));
		arabicon_print (ph, _(HELP));
		return;
	}
							 /*  00000 00000000 0000000 abc */
	arabicon_print (ph, _("\026 Ref#  Seconds  Repeat  Command \026\n"));
	list = timer_list;
	while (list)
	{
		tim = list->data;
		arabicon_printf (ph, _("%5d %8.1f %7d  %s\n"), tim->ref, tim->timeout / 1000.0f,
...
timer_plugin_init
#else
arabicon_plugin_init
#endif
				(arabicon_plugin *plugin_handle, char **plugin_name,
				char **plugin_desc, char **plugin_version, char *arg)
{
	...
	*plugin_version = "";
	...
	arabicon_hook_command (ph, "TIMER", HEXCHAT_PRI_NORM, timer_cb, _(HELP), 0);
