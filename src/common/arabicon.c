...
static int in_arabicon_exit = FALSE;
int arabicon_is_quitting = FALSE;
/* command-line args */
...
static int
arabicon_lag_check (void)   /* this gets called every 30 seconds */
{
	lag_check ();
	return 1;
}
...
static int
arabicon_lag_check_update (void)   /* this gets called every 0.5 seconds */
{
	lagcheck_update ();
	return 1;
}
...
arabicon_reinit_timers (void)
{
	...
	{
		lag_check_update_tag = fe_timeout_add (500, arabicon_lag_check_update, NULL);
	}
	...
	if ((prefs.hex_net_ping_timeout != 0 || prefs.hex_gui_lagometer)
	    && lag_check_tag == 0)
	{
		lag_check_tag = fe_timeout_add_seconds (30, arabicon_lag_check, NULL);
	}
...
session_free (session *killsess)
{
	...
	if (!sess_list && !in_arabicon_exit)
		arabicon_exit ();						/* sess_list is empty, quit! */
...
arabicon_exit (void)
{
	arabicon_is_quitting = TRUE;
	in_arabicon_exit = TRUE;
	plugin_kill_all ();
	fe_cleanup ();
...
{
#ifdef WIN32
	char arabicon_lang[13];	/* LC_ALL= plus 5 chars of hex_gui_lang and trailing \0 */

	strcpy (arabicon_lang, "LC_ALL=");

	if (0 <= prefs.hex_gui_lang && prefs.hex_gui_lang < LANGUAGES_LENGTH)
		strcat (arabicon_lang, languages[prefs.hex_gui_lang]);
	else
		strcat (arabicon_lang, "en");

	putenv (arabicon_lang);
#endif
}
