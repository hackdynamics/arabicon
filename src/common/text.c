...
text_convert_invalid (const gchar* text, gssize len, GIConv converter, const gchar *fallback, gsize *len_out)
{
	...
	for (;;)
	{
		 ...
		 *
		 * See https://github.com/arabicon/arabicon/issues/1758
		 */
...
pevent_make_pntevts (void)
{
	...
	for (i = 0; i < NUM_XP; i++)
	{
		...
		if (pevt_build_string (pntevts_text[i], &(pntevts[i]), &m) != 0)
		{
			...
			{
				g_error ("ARABICON CRITICAL *** default event text failed to build!");
			}
			else
			{
...
pevent_load (char *filename)
{
	...
	if (filename == NULL)
		fd = arabicon_open_file ("pevents.conf", O_RDONLY, 0, 0);
	else
		fd = arabicon_open_file (filename, O_RDONLY, 0, XOF_FULLPATH);
...
format_event (session *sess, int index, char **args, char *o, gsize sizeofo, unsigned int stripcolor_args)
{
	...
	while (done_all == FALSE)
	{
		...
		case 1:
			...
			{
				fprintf (stderr,
							"ARABICON DEBUG: display_event: arg > numargs (%d %d %s)\n",
							a, numargs, i);
				break;
			}
			ar = args[(int) a + 1];
...
pevent_save (char *fn)
{
	...
	if (!fn)
		fd = arabicon_open_file ("pevents.conf", O_CREAT | O_TRUNC | O_WRONLY,
									 0x180, XOF_DOMODE);
	else
		fd = arabicon_open_file (fn, O_CREAT | O_TRUNC | O_WRONLY, 0x180,
	...
	if (g_access (wavfile, R_OK) == 0)
	{
		...
		if (ca_con == NULL)
		{
			...
			ca_context_change_props (ca_con,
											CA_PROP_APPLICATION_ID, "arabicon",
											CA_PROP_APPLICATION_NAME, "ARABICON",
											CA_PROP_APPLICATION_ICON_NAME, "arabicon", NULL);
		}
...
sound_load ()
{
	...
	fd = arabicon_open_file ("sound.conf", O_RDONLY, 0, 0);
	if (fd == -1)
		return;
...
sound_save ()
{
	...
	fd = arabicon_open_file ("sound.conf", O_CREAT | O_TRUNC | O_WRONLY, 0x180,
								 XOF_DOMODE);
	if (fd == -1)
		return;
