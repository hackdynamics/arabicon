...
get_xdir (void)
{
	if (!xdir)
	{
		...
		else
		{
			...
			xdir = g_build_filename (roaming_path, "ARABICON", NULL);
...
default_file (void)
{
	...
	if (!dfile)
	{
		dfile = g_build_filename (get_xdir (), "arabicon.conf", NULL);
	}
	return dfile;
}
...
const struct prefs vars[] =
{
	...
	{"net_ping_timeout", P_OFFINT (hex_net_ping_timeout), TYPE_INT, arabicon_reinit_timers},
...
load_default_config(void)
{
	...
	memset (&prefs, 0, sizeof (struct arabiconprefs));
	...
	/* STRINGS */
	strcpy (prefs.hex_away_reason, _("أنا مشغول"));
	...
	safe_strcpy (prefs.hex_irc_part_reason, _("مع السلامة"), sizeof(prefs.hex_irc_part_reason));
...
int
arabicon_open_file (const char *file, int flags, int mode, int xof_flags)
{
	char *buf;
	int fd;
...
FILE *
arabicon_fopen_file (const char *file, const char *mode, int xof_flags)
{
	char *buf;
	FILE *fh;
