...
namespace thememan
{
    public partial class HTM : Form
    {
        ...
        public HTM ()
        {
            ...
            {
                arabicondir = Path.Combine(portableDir, "config\\");

                if (!Directory.Exists(arabicondir))
                {
                    MessageBox.Show("ARABICON installation not found!\nCheck your .\\config folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Environment.Exit(1);
                }
            }
            else
            {
                 ...
                 * Windows: %APPDATA%
                 * Unix: ~/.config
                 * Windows is case-insensitive so 'arabicon' should be fine for both
                 */
                arabicondir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "arabicon");

                if (!Directory.Exists(arabicondir))
                {
                    if (RunningOnWindows())
                    {
                        MessageBox.Show("ARABICON installation not found!\nCheck your %APPDATA%\\ARABICON folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);    
                    }
                    else
                    {
                        MessageBox.Show("ARABICON installation not found!\nCheck your ~/.config/arabicon folder", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
            ...
            themedir = Path.Combine(arabicondir, "themes");
            ListThemes();
        ...
        {
            DialogResult result = MessageBox.Show("ARABICON must be closed and this will overwrite your current theme!\n\nDo you wish to continue?", "Warning", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning);
            if (result == DialogResult.OK)
            {
                File.Copy(Path.Combine(themedir, themelist.SelectedItem.ToString(), "colors.conf"), Path.Combine(arabicondir, "colors.conf"), true);
                if (File.Exists(Path.Combine(themedir, themelist.SelectedItem.ToString(), "pevents.conf")))
                {
                    File.Copy(Path.Combine(themedir, themelist.SelectedItem.ToString(), "pevents.conf"), Path.Combine(arabicondir, "pevents.conf"), true);
                }
                else if (File.Exists(Path.Combine(arabicondir, "pevents.conf")))
                {
                    File.Delete(Path.Combine(arabicondir, "pevents.conf"));
                }
            }
        }
        ...
        private void importbutton_Click_1(object sender, EventArgs e)
        {
            ...
            importDialog.Filter = "ARABICON Theme Files|*.hct";
