...
#ifdef USE_DBUS
 {"existing",	'e', 0, G_OPTION_ARG_NONE,	&arg_existing, N_("Open URL or execute command in an existing ARABICON"), NULL},
#endif
...
#ifndef WIN32
	pixmaps_init ();
	gtk_window_set_icon (GTK_WINDOW (dialog), pix_arabicon);
#endif
...
const char cursor_color_rc[] =
	...
	"}"
	"widget \"*.arabicon-inputbox\" style : application \"xc-ib-st\"";

static const char adwaita_workaround_rc[] =
	"style \"arabicon-input-workaround\""
	"{"
		"engine \"pixmap\" {"
			"image {"
				"function = FLAT_BOX\n"
		...
		"}"
	"}"
	"widget \"*.arabicon-inputbox\" style \"arabicon-input-workaround\"";
...
#ifdef HAVE_GTK_MAC
	gtkosx_application_set_dock_icon_pixbuf (osx_app, pix_arabicon);
#endif
