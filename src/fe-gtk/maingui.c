...
/*	printf("enter mg_topdestroy. sess %p was destroyed\n", sess);*/
	session_free (sess);	/* tell arabicon.c about it */
}
...
mg_open_quit_dialog (gboolean minimize_button)
{
	...
	gtk_window_set_title (GTK_WINDOW (dialog), _("Quit ARABICON?"));
...
mg_create_chanmodebuttons (session_gui *gui, GtkWidget *box)
{
	...
	gtk_widget_set_name (gui->key_entry, "arabicon-inputbox");
	...
	gtk_widget_set_name (gui->limit_entry, "arabicon-inputbox");
...
mg_create_topicbar (session *sess, GtkWidget *box)
{
	...
	gtk_widget_set_name (topic, "arabicon-inputbox");
...
mg_create_entry (session *sess, GtkWidget *box)
{
	...
	gtk_widget_set_name (entry, "arabicon-inputbox");
...
mg_create_topwindow (session *sess)
{
	...
	if (sess->type == SESS_DIALOG)
		win = gtkutil_window_new ("ARABICON", NULL,
...
mg_create_tabwindow (session *sess)
{
	...
	win = gtkutil_window_new ("ARABICON", NULL, prefs.hex_gui_win_width,
									  prefs.hex_gui_win_height, 0);
	sess->gui->window = win;
