...
#include "../common/arabicon.h"
#include "../common/arabiconc.h"
...
tray_get_window_status (void)
{
	...
	st = arabicon_get_info (ph, "win_status");
...
tray_toggle_visibility (gboolean force_hide)
{
	...
	arabicon_set_context (ph, arabicon_find_context (ph, NULL, NULL));

	win = GTK_WINDOW (arabicon_get_info (ph, "gtkwin_ptr"));
	...
	if (force_hide || gtk_widget_get_visible (GTK_WIDGET (win)))
	{
		if (prefs.hex_gui_tray_away)
			arabicon_command (ph, "ALLSERV AWAY");
	...
	{
		if (prefs.hex_gui_tray_away)
			arabicon_command (ph, "ALLSERV BACK");
	...
	if (prefs.hex_input_tray_hilight)
	{
		...
		if (tray_hilight_count == 1)
			tray_set_tipf (_("Highlighted message from: %s (%s) - %s"),
								word[1], arabicon_get_info (ph, "channel"), _(DISPLAY_NAME));
		else
			tray_set_tipf (_("%u highlighted messages, latest from: %s (%s) - %s"),
								tray_hilight_count, word[1], arabicon_get_info (ph, "channel"),
								_(DISPLAY_NAME));
	}
...
tray_priv (char *from, char *text)
{
	...
	network = arabicon_get_info (ph, "network");
	if (!network)
...
tray_apply_setup (void)
{
	...
	{
		GtkWindow *window = GTK_WINDOW(arabicon_get_info (ph, "gtkwin_ptr"));
...
int
tray_plugin_init (arabicon_plugin *plugin_handle, char **plugin_name,
				char **plugin_desc, char **plugin_version, char *arg)
{
	/* we need to save this for use with any arabicon_* functions */
	ph = plugin_handle;
	...
	*plugin_version = "";
	...
	arabicon_hook_print (ph, "Channel Msg Hilight", -1, tray_hilight_cb, NULL);
	arabicon_hook_print (ph, "Channel Action Hilight", -1, tray_hilight_cb, NULL);

	arabicon_hook_print (ph, "Channel Message", -1, tray_message_cb, NULL);
	arabicon_hook_print (ph, "Channel Action", -1, tray_message_cb, NULL);
	arabicon_hook_print (ph, "Channel Notice", -1, tray_message_cb, NULL);

	arabicon_hook_print (ph, "Private Message", -1, tray_priv_cb, NULL);
	arabicon_hook_print (ph, "Private Message to Dialog", -1, tray_priv_cb, NULL);
	arabicon_hook_print (ph, "Private Action", -1, tray_priv_cb, NULL);
	arabicon_hook_print (ph, "Private Action to Dialog", -1, tray_priv_cb, NULL);
	arabicon_hook_print (ph, "Notice", -1, tray_priv_cb, NULL);
	arabicon_hook_print (ph, "Invited", -1, tray_invited_cb, NULL);

	arabicon_hook_print (ph, "DCC Offer", -1, tray_dcc_cb, NULL);

	arabicon_hook_print (ph, "Focus Window", -1, tray_focus_cb, NULL);
...
int
tray_plugin_deinit (arabicon_plugin *plugin_handle)
{
#ifdef WIN32
	tray_cleanup ();
#endif
