...
key_save_kbs (void)
{
	...
	fd = arabicon_open_file ("keybindings.conf", O_CREAT | O_TRUNC | O_WRONLY,
									 0x180, XOF_DOMODE);
	if (fd < 0)
		return 1;
	write (fd, buf, g_snprintf (buf, 510, "# ARABICON key bindings config file\n\n"));
...
key_load_kbs (void)
{
	...
	fd = arabicon_open_file ("keybindings.conf", O_RDONLY, 0, 0);
	if (fd < 0)
	{
